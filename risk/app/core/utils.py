...
from ..constants import MonitorStatus, SearchStatus, ApproveStatus
...
def query_address(location, apikey=Config.GD_KEY):
    """获取高德地图坐标的地址"""

    url1 = "http://restapi.amap.com/v3/assistant/coordinate/convert"
    params1 = {
        "key": apikey,
        "locations": location,
        "coordsys": "gps",
    ...
    try:
        r = requests.get(url1, params=params1, timeout=5)
        r_json = r.json()
        real_location = r_json["locations"]
        project_logger.info('location got: {0}'.format(r_json))
    except:
        traceback.print_exc()
        return ''

    url2 = "http://restapi.amap.com/v3/geocode/regeo"
    params2 = {
        "key": apikey,
    ...
    try:
        ...
        r_json = r.json()
        address = r_json["regeocode"]["formatted_address"]
        project_logger.info('address got: {0}'.format(r_json))
    except:
        traceback.print_exc()
        return ''
